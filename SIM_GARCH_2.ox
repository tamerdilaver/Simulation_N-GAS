...
fEstimateGarch(const vReturns, const adAlpha_hat, const adBeta_hat, const adOmega_hat, const adGamma_hat){
	...
	vTheta = <0.1 ; 0.89 ; 0.01 ; 0.1>; // Alpha, Beta, Omega, Gamma Startingvalues
...
fMonteCarlo(const amMonteCarlo){
	...
	for(decl i = 0; i<iB ; i++){
		...
		mTemp[i][0]	=  (dAlpha_hat-dALPHA)	/vSE[0];				//(\hat_\alpha_T - \alpha_0)/SE(\hat_\alpha) ~ N(0, 1)
